...
class BookmarkStore(object):
    BOOKMARKS_YAML_FILENAME = "bookmarks.yaml"
    (STATE_IDLE,
     STATE_WAITING_FOR_RESPONSE) = range(2)

    def __init__(self,
                 list_bookmarks_callback,
                 connected_to_cloud_callback,
                 disconnected_from_cloud_callback,
                 browser_authentication_needed_callback):
        self._bookmarks = None
        self._list_bookmarks_external_callback = list_bookmarks_callback
        self._browser_authentication_needed_external_callback = browser_authentication_needed_callback
        self._cloud_bookmarks_syncer = cloudfilesynchronizerthread.CloudFileSynchronizerThread(
                self.BOOKMARKS_YAML_FILENAME,
                connected_to_cloud_callback,
                disconnected_from_cloud_callback,
    ...
    def connect_to_drive_explicitly(self):
        self._cloud_bookmarks_syncer.try_to_connect_explicitly()

    def is_connected_to_drive(self):
        self._cloud_bookmarks_syncer.is_connected_to_drive()
    ...
    def _list_bookmarks_callback(self, bookmarks_yaml):
        ...
        self._update_state(bookmarks_yaml)
    ...
    def _explicit_authentication_needed_callback(self):
        ...
        self._browser_authentication_needed_external_callback()

    def _update_state(self, encoded_yaml):
        self._bookmarks = [{'guid': 'ROOT', 'children': []}]
        try:
            decoded = yaml.safe_load(encoded_yaml)
            children = decoded[0]['children']
            self._bookmarks[0]['children'].extend(children)
        except:
    ...
    def _async_write(self):
        ...
        self._cloud_bookmarks_syncer.async_write(contents)
